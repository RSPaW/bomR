...
dir.create(here("tmax-mean"))
dir.create(here("tmax-mean", "01day"))

server <- "http://opendap.bom.gov.au:8080/thredds/fileServer/agcd/tmax/mean/r005/01day/"

yrs <- 1910:year(Sys.Date())
i<- 1
...
#for (i in 1:length(yrs)) {
  dir.create(here("tmax-mean", "01day", yrs[i]), showWarnings = FALSE)
  m <- 1
  for (m in 1:12){
    if(m < 10){
      mnth <- paste0(0, m)
    ...
    for (d in 1:31){
      ...
      filename <- paste0(yrs[i], "/tmax_mean_r005_", yrs[i], mnth, dy, "_",  yrs[i], mnth, dy,".nc" ) 
      destfile <- here("tmax-mean", "01day", filename)
